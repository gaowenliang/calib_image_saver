...
#include "../../include/calib_image_saver/chessboard/ChessboardQuad.h"
#include "../../include/calib_image_saver/chessboard/Spline.h"
...
//          cv::cvtColor( image, norm_img, CV_BGR2GRAY );
    ...
    for ( int k = 0; k < 6; ++k )
    {
        for ( int dilations = minDilations; dilations <= maxDilations; ++dilations )
        {
            ...
            {
                int blockSize
                = lround( prevSqrSize == 0 ? std::min( img.cols, img.rows ) * ( k % 2 == 0 ? 0.2 : 0.1 ) : prevSqrSize * 2 )
                  | 1;

                // convert to binary
                cv::adaptiveThreshold( img, thresh_img, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY,
                                       blockSize, ( k / 2 ) * 5 );
            }
            else
            {
...
Chessboard::generateQuads( std::vector< ChessboardQuadPtr >& quads, cv::Mat& image, int flags, int dilation, bool firstRun )
{
    ...
    for ( size_t i = 0; i < contours.size( ); ++i )
    {
        ...
        if ( approxContour.size( ) == 4 && cv::isContourConvex( approxContour ) )
        {
            ...
            if ( !( flags & CV_CALIB_CB_FILTER_QUADS ) || ( d3 * 4 > d4 && d4 * 4 > d3 && d3 * d4 < area * 1.5
